...
import { AllowListEntry } from '@/lib/utils/merkle-proof'
import { formatCryptoVal } from '@/lib/utils/numbers'

import { MintButton } from './MintButton'

import { OPEN_EDITION_SIZE } from '@/lib/constants/numbers'
...
export function MintComponent({
  ...
  const internalPrice =
    allowlistEntry?.price || collection.salesConfig.publicSalePrice
  ...
  const clampMintCounter = useCallback(() => {
    if (mintCounter > availableMints)
      setMintCounter(Math.max(1, availableMints))
    else if (mintCounter > availableTokenCount)
      setMintCounter(availableTokenCount)
  ...
  return (
    <>
      ...
      className={[
        className,
        'zord-mint-status',
        presale && 'zord-mint-status--presale',
      ]}
      gap='x4'
      {...props}
      > */}
      {showPrice && !saleIsFinished && !isSoldOut && (
        ...
        > */}
          ...
          {internalPrice === '0'
            ? 'Free'
            : `${formatCryptoVal(
                Number(internalPrice) * (mintCounter || 1)
              )} ETH`}
          {/* </Heading> */}
          {/* </Stack> */}
          ...
          {saleIsActive && !isSoldOut ? (
            <>
              ...
              <input
                ...
                value={mintCounter || ''}
                onChange={(e) =>
                  handleMintCounterUpdate(Number(e.target.value))
                }
              ...
              {/* <Button
                  w='x12'
                  disabled={
                    mintCounter >= Math.min(availableMints, availableTokenCount)
                  }
                  variant='circle'
                  onClick={() =>
                    setMintCounter((state) =>
                    state < maxPerWallet ? state + 1 : state
                    )
                  }
                  > */}
              {/* <Heading size='sm' className={priceDateHeading}> */}+
