...
export const SaleHistory = ({ showPerpetual = true, className }: SaleHistoryProps) => {
  ...
  const getPastBids = () => {
    ...
    //   (data.pricing.reserve.status === "Active" || data.pricing.reserve.status === "Finished")
    // ) {
    ...
    //   const unixDate = new Date(data.zoraIndexerResponse.mintTransferEvent?.blockTimestamp + "Z").getTime() / 1000;
    ...
    //     transactionHash: data.zoraIndexerResponse.mintTransferEvent?.transactionHash || null,
    //   });
    // }
